...
6. Status transitions follow `src/lib/invoices/status.ts`. A daily cron (`/api/cron/overdue`, 06:00 UTC, requires `CRON_SECRET`) flips past-due sent or opened invoices to overdue. Merchants send reminders from the invoice page: client on file only, one per invoice per 24 hours (decision 0015). The dashboard shows Outstanding, Overdue, Paid this month, plus aging buckets (current, 1 to 30, 31 to 60, 60+ days) net of partial payments.
