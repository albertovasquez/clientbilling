...
9. API: personal keys at `/app/settings/api`, endpoints under `/api/v1/*`, reference at `/docs/api` and `docs/agents/api.md` (decisions 0017, 0025). State-changing calls require `Idempotency-Key`. Material mutations append `BillingEvent` rows (actor + hash chain) and `InvoiceVersion` snapshots; `InvoiceEvent` remains for UI and cooldowns. Invoice rules live in `src/lib/invoices/service.ts` and `src/lib/invoices/email.ts`, shared by the app and the API.
