...
The identity is Carbon Copy (`docs/brand/brief.md`, decision 0021). Tokens are defined in `src/app/globals.css` under `@theme`; pages use only the semantic utilities. Raw palette classes are allowed only in `src/components/ui/`. The retired names `action`, `action-hover`, `action-tint`, `verdict`, `verdict-tint`, and `verdict-rule` were removed in issue #49; the style check fails on them, because a retired utility resolves to nothing and loses its color silently.
...
| rule-due | #F1C9B8 | `border-rule-due` | the hairline that pairs with due-tint on a callout |
...
- Phone width means 390 px and 320 px without horizontal scroll, with the menu open as well as closed. `scripts/test-phone-width.ts` (`npm run test:phone`) drives headless Chrome against a built server, opens the disclosure, and compares `scrollWidth` to `clientWidth` on the homepage, `/payments`, and `/invoices`. It needs a running server, so it sits beside `test:db` rather than inside `npm run check`. At 320 px the row trades padding, gaps, and one step down in the wordmark's size below 360 px for the space it needs; the button keeps `text-small`. The mark, the site name, and the button all stay visible, and the name is never truncated: a wordmark reading "Client..." is not the site name. The check asserts all three render in full, so truncating to fit fails.
